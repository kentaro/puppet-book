...
先に説明した通り、out_fileプラグインは`time_slice_format`で指定した期間の単位でログファイルを作成するので、このファイルは翌日(正確には、この場合4/21の0:00を過ぎた後に)、以下のようなファイルに書き出されます。

```
...
```

Puppetでtd-agentのクラスタ構成を記述し、動作確認ができました。
