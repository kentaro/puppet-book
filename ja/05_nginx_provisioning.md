...
`$port = 80`や`$target = "Puppet"`という箇所は、見ての通り、変数への代入を行っています。Puppetではこのように、manifestの中で変数を使えます。後述の`template()`といっしょに説明します。
...
この`<%= port %>`の箇所が、erbの変数展開の記法として評価され、manifestで`$port = 80`と記述されている内容がうめこまれます。
